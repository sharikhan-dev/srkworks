...
export function ProjectCard({ project, index }: ProjectCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group relative rounded-2xl sm:rounded-[2.5rem] overflow-hidden glass-surface glass-surface-hover border border-white/10 p-4 sm:p-8 transition-all duration-700"
    >
      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8 items-center ${isEven ? '' : 'lg:flex-row-reverse'}`}>
        ...
        <div className={`lg:col-span-7 relative rounded-xl sm:rounded-3xl overflow-hidden aspect-[16/10] sm:aspect-[16/9] bg-neutral-900/80 border border-white/5 ${isEven ? 'lg:order-1' : 'lg:order-2'}`}>
          {imageUrl && !imgError ? (
            <img
              src={imageUrl}
              alt={title}
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-6 sm:p-8 text-center">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl glass-surface border border-white/10 flex items-center justify-center text-neutral-400 mb-3">
          ...
          <div className="absolute top-3 sm:top-4 left-3 sm:left-4 right-3 sm:right-4 flex items-center justify-between pointer-events-none">
            <span className="glass-pill px-2.5 sm:px-3 py-1 rounded-full text-[10px] sm:text-[11px] font-semibold tracking-wider uppercase text-neutral-200 border border-white/15">
              {category}
            </span>

            <span className="text-[11px] sm:text-xs font-mono text-neutral-300 glass-pill px-2.5 py-1 rounded-full border border-white/10">
              {year}
            </span>
          </div>
        </div>
        ...
        <div className={`lg:col-span-5 flex flex-col justify-between space-y-4 sm:space-y-6 ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
          <div>
            <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
              <span className="text-[11px] sm:text-xs font-mono uppercase tracking-widest text-neutral-400">
                {category} · {year}
              </span>
            </div>
            ...
            {description && (
              <p className="text-xs sm:text-base text-neutral-300/90 leading-relaxed mt-2.5 sm:mt-3 font-normal">
                {description}
              </p>
            )}
          </div>
